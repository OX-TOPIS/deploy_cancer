...
const DisplayBloodResults = () => {
  ...
  useEffect(() => {
    const fetchFormula = async () => {
      try {
        ...
        const response = await fetch(`http://3.107.2.16:8080/patientss/${username}`);
        if (response.ok) {
  ...
  useEffect(() => {
    const fetchFormula = async () => {
      try {
        console.log(`Fetching formula for user: ${username}`);
        const response = await fetch(`http://3.107.2.16:8080/formulas/${username}`);
        if (response.ok) {
          const data = await response.json();
          console.log('Data received from API:', data);
          if (data.length > 0) {
  ...
  useEffect(() => {
    const fetchCancerTypes = async () => {
      try {
        ...
        const response = await fetch(`http://3.107.2.16:8080/cancerType/${username}`);
        if (response.ok) {
          const data = await response.json();
  ...
  useEffect(() => {
    const fetchCancerTypes = async () => {
      try {
        console.log(`Fetching cancer types for user: ${username}`);
        const response = await fetch(`http://3.107.2.16:8080/recordbloodresult/${username}`);
        if (response.ok) {
          const data = await response.json();
            ...
            {recordBlooadResult.map((recordBlooad, index) => (
              ...
              <div className="box-sd mb-2">
                <div className="">
                  ...
                  <img src={"http://3.107.2.16:8080/" + recordBlooad.picture} className="w-20 h-20" 
                  onClick={() => openPopup("http://3.107.2.16:8080/" + recordBlooad.picture)}
                  />
